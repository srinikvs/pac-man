...
function TitleOverlay({ highScore, onStart }: { highScore: number; onStart: () => void }) {
  ...
  return (
    <div className="overlay overlay-title">
      <p className="overlay-kicker">HIGH SCORE {String(highScore).padStart(2, "0")}</p>
      <h2 className="overlay-logo">Pac-Man</h2>
      <p className="overlay-tag">Maze chase. One life at a time.</p>
      <ul className="ghost-list">
        {ghosts.map((id) => (
          <li key={id} className="ghost-row">
            <GhostMark color={GHOST_COLOR[id]} />
            <span className="ghost-nick">{GHOST_NICK[id].nick}</span>
            <span className="ghost-name">{GHOST_NICK[id].name}</span>
          </li>
        ))}
      </ul>
      ...
      <p className="overlay-fine">Enter or tap · Pause with P · Mute with M</p>
    </div>
  );
}
  ...
  return (
    <div className="overlay overlay-modal">
      <div className="modal">
        ...
        <h2 className="modal-title">{String(score).padStart(2, "0")}</h2>
        <p className="modal-copy">Best {String(highScore).padStart(2, "0")}</p>
